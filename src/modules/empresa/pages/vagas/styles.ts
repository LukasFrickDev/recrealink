import styled from "styled-components";

export const Wrapper = styled.div`
  display: grid;
  gap: 18px;
`;

export const PrimaryAction = styled.button`
  border: 1px solid #cbb1f4;
  background: linear-gradient(120deg, #8a61d4 0%, #6f4ab2 100%);
  color: #ffffff;
  border-radius: ${({ theme }) => theme.radii.md};
  min-height: 36px;
  padding: 0 12px;
  font-size: 12px;
  font-weight: 700;
  cursor: pointer;

  &:hover {
    filter: brightness(0.96);
  }
`;

export const MainGrid = styled.div`
  display: grid;
  gap: 12px;
  grid-template-columns: repeat(2, minmax(0, 1fr));

  @media (max-width: ${({ theme }) => theme.breakpoints.desktop}) {
    grid-template-columns: minmax(0, 1fr);
  }
`;

export const PositionList = styled.div`
  display: grid;
  gap: 10px;
`;

export const PositionItem = styled.article`
  border: 1px solid ${({ theme }) => theme.colors.border};
  border-radius: ${({ theme }) => theme.radii.md};
  padding: 12px;
  display: grid;
  gap: 6px;

  header {
    display: flex;
    justify-content: space-between;
    gap: 10px;
    align-items: center;
  }

  strong {
    font-size: 14px;
  }

  p {
    font-size: 12px;
    color: ${({ theme }) => theme.colors.textMuted};
  }
`;

export const PositionMeta = styled.div`
  display: flex;
  flex-wrap: wrap;
  gap: 8px;

  span {
    font-size: 11px;
    border: 1px solid #e4d8f8;
    border-radius: ${({ theme }) => theme.radii.pill};
    padding: 3px 8px;
    background: #faf6ff;
  }
`;

export const PositionApplications = styled.span`
  font-size: 12px;
  font-weight: 700;
  color: #6f4ab2;
`;

export const ApplicationList = styled.div`
  display: grid;
  gap: 10px;
`;

export const ApplicationItem = styled.article`
  border: 1px solid #e7ddf8;
  border-radius: ${({ theme }) => theme.radii.md};
  background: #fcfaff;
  padding: 12px;
  display: grid;
  gap: 6px;

  header {
    display: flex;
    justify-content: space-between;
    gap: 10px;
    align-items: center;
  }

  strong {
    font-size: 14px;
  }

  p {
    font-size: 12px;
    color: ${({ theme }) => theme.colors.textMuted};
  }
`;

export const ApplicationMeta = styled.div`
  display: flex;
  justify-content: space-between;
  gap: 8px;

  span {
    font-size: 11px;
    font-weight: 700;
    color: #6f4ab2;
  }
`;

export const ApplicationHighlight = styled.span`
  font-size: 12px;
  color: ${({ theme }) => theme.colors.text};
`;

export const BottomGrid = styled.div`
  display: grid;
  gap: 12px;
  grid-template-columns: repeat(2, minmax(0, 1fr));

  @media (max-width: ${({ theme }) => theme.breakpoints.desktop}) {
    grid-template-columns: minmax(0, 1fr);
  }
`;

export const StageList = styled.div`
  display: grid;
  gap: 10px;
`;

export const StageItem = styled.article`
  border-left: 4px solid #8a61d4;
  border-radius: ${({ theme }) => theme.radii.md};
  background: #f8f2ff;
  padding: 10px 12px;
  display: grid;
  gap: 4px;

  strong {
    font-size: 13px;
  }

  p {
    font-size: 12px;
    color: ${({ theme }) => theme.colors.textMuted};
  }
`;

export const RecommendationList = styled.ul`
  margin: 0;
  padding: 0;
  list-style: none;
  display: grid;
  gap: 8px;
`;

export const RecommendationItem = styled.li`
  border: 1px solid #eadffb;
  border-radius: ${({ theme }) => theme.radii.md};
  background: #fcfaff;
  padding: 10px 12px;
  font-size: 13px;
  color: ${({ theme }) => theme.colors.text};
`;
